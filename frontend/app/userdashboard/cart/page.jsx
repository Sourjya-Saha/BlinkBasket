...
function SpinnerRing({ size = 16, color = 'currentColor' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" style={{ animation: 'cp-spin 0.7s linear infinite' }}>
      ...
      <path d="M4.93 4.93l2.83 2.83" strokeOpacity="0.75"/>
      <path d="M16.24 16.24l2.83 2.83" strokeOpacity="0.2"/>
      <path d="M2 12h4" strokeOpacity="0.6"/>
      <path d="M18 12h4" strokeOpacity="0.15"/>
      <path d="M4.93 19.07l2.83-2.83" strokeOpacity="0.4"/>
      <path d="M16.24 7.76l2.83-2.83" strokeOpacity="0.9"/>
    </svg>
  );
}
...
function StepBar({ current }) {
  ...
  // Calculate percentage to fill the background line correctly
...
// ── Address step (inline, no separate file) ───────────────────────────────────
...
function AddressStep({ onConfirm, onBack }) {
  ...
  function selectSuggestion(s) {
    const short = [s.address?.road, s.address?.suburb, s.address?.city || s.address?.town]
      .filter(Boolean).join(', ') || s.display_name;
  ...
  return (
    <div className="cp-addr-wrap">
      ...
      <button className="cp-locate-btn" onClick={autoLocate} disabled={locating} type="button">
        <span className="cp-locate-icon-box">
          {locating ? <SpinnerRing size={14} color="#a3e635" /> : <LocateIcon />}
        </span>
      ...
      <div className="cp-addr-search-wrap">
        <div className={`cp-addr-input-row${inputFocused ? ' focused' : ''}`}>
          ...
          {query && !suggLoading && (
            <button className="cp-addr-clear" type="button" onMouseDown={e => {
              e.preventDefault();
              setQuery(''); setSuggestions([]); setSelectedLocation(null); setDeliveryStatus(null);
      ...
      {deliveryStatus && (
        <div className={`cp-delivery-banner ${deliveryStatus}`}>
          {deliveryStatus === 'checking' && (
            <><SpinnerRing size={17} /><div><div className="cp-del-title">Checking delivery availability…</div></div></>
          )}
          {deliveryStatus === 'available' && (
            <><CheckCircleIcon /><div><div className="cp-del-title">We deliver here! 🎉</div><div className="cp-del-sub">Fill in your details below to continue</div></div></>
          )}
          {deliveryStatus === 'unavailable' && (
            <><XCircleIcon /><div><div className="cp-del-title">Delivery not available yet</div><div className="cp-del-sub">We're expanding — try a nearby location</div></div></>
          )}
        </div>
      )}

      {/* ── Address fields (only when serviceable) ── */}
      {deliveryStatus === 'available' && (
        <div className="cp-addr-fields">
          ...
          <div className="cp-addr-field">
            <label className="cp-addr-lbl">Flat / House No. *</label>
            <input className="cp-addr-finput" type="text" placeholder="e.g. 12B, Tower C" value={flatNo} onChange={e => setFlatNo(e.target.value)} />
          </div>
          <div className="cp-addr-field">
            <label className="cp-addr-lbl">Landmark <span style={{ color: '#4b5563', fontWeight: 400 }}>(optional)</span></label>
            <input className="cp-addr-finput" type="text" placeholder="Near metro, park, school…" value={landmark} onChange={e => setLandmark(e.target.value)} />
          </div>
          <div className="cp-addr-row2">
            <div className="cp-addr-field">
              <label className="cp-addr-lbl">Pincode *</label>
              <input className="cp-addr-finput" type="text" placeholder="6-digit pincode" maxLength={6} value={pincode} onChange={e => setPincode(e.target.value.replace(/\D/g, '').slice(0, 6))} />
            </div>
            <div className="cp-addr-field">
              <label className="cp-addr-lbl">City</label>
              <input className="cp-addr-finput" style={{ opacity: 0.55, cursor: 'default' }} type="text" readOnly value={selectedLocation?.short?.split(',').pop()?.trim() || ''} placeholder="Auto-filled" />
            </div>
          </div>
        </div>
      )}
      ...
      <div className="cp-addr-nav">
        ...
        <button className="cp-cta-btn" disabled={!canProceed} onClick={handleContinue} type="button">
          Continue to Payment <ChevronRightIcon />
        </button>
      </div>
    </div>
  );
...
export default function CartPage() {
  ...
  // Address step state (lifted up so it persists when going back)
  const [addressData, setAddressData] = useState(null); // { delivery_address, pincode, lat, lng }
  ...
  const fetchCart = useCallback(async () => {
    ...
    try {
      ...
      // FRONTEND BAND-AID FIX: Grouping Duplicate Items
      // Note: Make sure to fix this in your backend POST route!
      const mergedItems = [];
      const map = new Map();

      fetchedItems.forEach(item => {
        const productId = item.products?.id;
        const variantId = item.product_variants?.id || 'base';
  ...
  function getProductPrice(item) {
    return parseFloat(item.product_variants?.price ?? item.products?.price) || 0;
  }
  ...
  async function placeOrder() {
    ...
    if (paymentMethod === 'cod') {
      ...
      try {
        const res = await fetch(`${API_BASE}/orders/cod`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.accessToken}` },
          body: JSON.stringify({ delivery_address, pincode, total: total.toFixed(2) }),
        });
        const result = await res.json();
        if (result.success) {
          showToast('success', 'Order placed!');
          setCartItems([]); setCartCount(0); setCheckoutStep('cart');
    ...
    try {
      const createRes = await fetch(`${API_BASE}/orders/razorpay/create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.accessToken}` },
      ...
      const rz = new window.Razorpay({
        ...
        amount: orderRes.amount, currency: orderRes.currency || 'INR',
        name: 'BlinkBasket', description: 'Order Payment',
        order_id: orderRes.razorpay_order_id,
        handler: async (response) => {
          const verRes = await fetch(`${API_BASE}/orders/razorpay/verify`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.accessToken}` },
            body: JSON.stringify({ ...response, delivery_address, pincode, total: total.toFixed(2) }),
          });
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cp-main {
          ...
          max-width: 700px; margin: 0 auto; padding: 0 1rem 4rem;
        }
        ...
        .cp-page-title {
          font-family: 'Syne', sans-serif; font-size: clamp(1.25rem, 3vw, 1.55rem);
          font-weight: 800; color: #fff; letter-spacing: -0.03em;
        }

        /* ── PROGRESS STEP BAR ── */
        .cp-progress-container {
          margin-bottom: 2rem; position: relative; padding: 0 1rem; margin-top: 0.5rem;
        }
        .cp-progress-track {
        ...
        .cp-progress-fill {
          height: 100%; background: #a3e635; transition: width 0.3s ease;
        }
        .cp-progress-steps {
        ...
        .cp-prog-step.done .cp-prog-dot {
          background: #a3e635; border-color: #a3e635; color: #052e16;
        }
        .cp-prog-step.active .cp-prog-dot {
          border-color: #a3e635; color: #a3e635; box-shadow: 0 0 12px rgba(163,230,53,0.25);
        }
        .cp-prog-label {
          font-size: 0.7rem; font-weight: 700; color: #6b7280; text-transform: uppercase;
          letter-spacing: 0.05em; transition: color 0.3s ease; white-space: nowrap;
        }
        .cp-prog-step.done .cp-prog-label, .cp-prog-step.active .cp-prog-label {
          color: #e5e7eb;
        }
        ...
        /* ── GLASS CARD ── */
        .cp-glass {
          background: rgba(13,16,24,0.7); backdrop-filter: blur(14px);
          border: 1px solid rgba(255,255,255,0.07); border-radius: 18px; padding: 1.5rem;
        }
        ...
        .cp-item-img {
          ...
          background: #0c111a; flex-shrink: 0; border: 1px solid rgba(255,255,255,0.07);
        }
        .cp-item-body { flex: 1; display: flex; flex-direction: column; gap: 0.35rem; }
        .cp-item-name {
          font-family: 'Syne', sans-serif; font-size: 0.95rem; font-weight: 700; color: #f3f4f6; line-height: 1.3;
        }
        .cp-item-variant {
        ...
        .cp-item-price {
          font-family: 'Syne', sans-serif; font-size: 1.05rem; font-weight: 800; color: #a3e635; margin-top: 0.2rem;
        }
        ...
        .cp-qty-btn {
          ...
          background: rgba(163,230,53,0.1); border: 1px solid rgba(163,230,53,0.18);
        ...
        .cp-rm-btn {
          ...
          background: rgba(239,68,68,0.08); border: 1px solid rgba(239,68,68,0.18);
          color: #f87171; font-size: 0.75rem; font-weight: 600; font-family: 'DM Sans', sans-serif; cursor: pointer;
          transition: background 0.13s;
        }
        .cp-rm-btn:hover { background: rgba(239,68,68,0.15); }

        /* ── DETAILED BILL AT BOTTOM ── */
        .cp-bill-details { padding: 1.5rem; }
        .cp-bill-title {
          font-family: 'Syne', sans-serif; font-size: 1.1rem; font-weight: 700; color: #fff;
          margin-bottom: 1.2rem; padding-bottom: 0.8rem; border-bottom: 1px dashed rgba(255,255,255,0.15);
        }
        .cp-bill-row {
          display: flex; justify-content: space-between; margin-bottom: 0.85rem; font-size: 0.9rem;
        }
        ...
        .cp-free-banner {
          ...
          background: rgba(163,230,53,0.07); border: 1px dashed rgba(163,230,53,0.3);
        ...
        .cp-cta-btn {
          ...
          padding: 1rem 1.2rem; border-radius: 13px; border: none;
          background: linear-gradient(135deg, #a3e635 0%, #84cc16 100%);
          color: #052e16; font-family: 'Syne', sans-serif; font-size: 0.95rem; font-weight: 700;
          letter-spacing: 0.02em; cursor: pointer; transition: all 0.18s ease; width: 100%;
        }
        .cp-cta-btn:hover:not(:disabled) {
          transform: translateY(-2px); box-shadow: 0 8px 28px rgba(163,230,53,0.3);
        }
        ...
        .cp-ghost-btn {
          ...
          padding: 1rem 1.1rem; border-radius: 13px;
          background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.09);
          color: #9ca3af; font-family: 'DM Sans', sans-serif; font-size: 0.9rem; font-weight: 600;
        ...
        .cp-addr-section-lbl {
          ...
          text-transform: uppercase; color: #6b7280; margin-bottom: 0.7rem;
        }
        .cp-locate-btn {
          width: 100%; display: flex; align-items: center; gap: 12px;
          padding: 1rem 1.2rem; border-radius: 13px; cursor: pointer;
          background: rgba(163,230,53,0.05); border: 1.5px dashed rgba(163,230,53,0.28);
          color: #a3e635; font-family: 'DM Sans', sans-serif; font-size: 0.95rem; font-weight: 600;
          transition: all 0.18s; margin-bottom: 1.2rem;
        }
        ...
        .cp-locate-icon-box {
          ...
          background: rgba(163,230,53,0.15); display: flex; align-items: center; justify-content: center; flex-shrink: 0;
        }
        ...
        .cp-addr-input-row {
          display: flex; align-items: center; gap: 10px; padding: 0 1.1rem;
          background: rgba(13,16,24,0.85); border: 1.5px solid rgba(255,255,255,0.08);
        ...
        .cp-addr-input {
          ...
          padding: 1rem 0; font-family: 'DM Sans', sans-serif; font-size: 0.95rem; color: #f3f4f6;
        }
        ...
        .cp-addr-dropdown {
          ...
          background: rgba(9,11,19,0.99); border: 1px solid rgba(255,255,255,0.1);
          border-radius: 13px; overflow: hidden;
          box-shadow: 0 24px 64px rgba(0,0,0,0.7); animation: cp-drop 0.15s ease;
        }
        .cp-addr-sugg {
          display: flex; align-items: flex-start; gap: 10px; padding: 0.9rem 1.1rem;
          cursor: pointer; transition: background 0.1s; border-bottom: 1px solid rgba(255,255,255,0.04);
        }
        .cp-addr-sugg:last-child { border-bottom: none; }
        .cp-addr-sugg:hover { background: rgba(163,230,53,0.07); }
        .cp-addr-sugg-pin {
          width: 28px; height: 28px; border-radius: 7px; background: rgba(163,230,53,0.1);
          color: #a3e635; display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 1px;
        }
        ...
        .cp-addr-pill {
          ...
          padding: 0.75rem 1rem; background: rgba(163,230,53,0.06);
          border: 1px solid rgba(163,230,53,0.2); border-radius: 10px;
          margin-bottom: 1rem; animation: cp-fadein 0.2s ease;
        }
        ...
        .cp-delivery-banner {
          ...
          border-radius: 12px; border: 1px solid; margin-bottom: 1.2rem;
        ...
        .cp-addr-finput {
          ...
          background: rgba(13,16,24,0.85); border: 1.5px solid rgba(255,255,255,0.08);
          border-radius: 11px; font-family: 'DM Sans', sans-serif; font-size: 0.95rem; color: #f3f4f6;
          outline: none; transition: all 0.17s;
        }
        ...
        @media (max-width: 480px) { .cp-addr-row2 { grid-template-columns: 1fr; } }
        ...
        .cp-addr-review {
          padding: 1rem 1.2rem; background: rgba(163,230,53,0.07);
          border: 1px solid rgba(163,230,53,0.2); border-radius: 12px; margin-bottom: 1.5rem;
        }
        ...
        .cp-pay-opt {
          display: flex; align-items: center; gap: 1.2rem; padding: 1.2rem 1.2rem;
          background: rgba(255,255,255,0.03); border: 1.5px solid rgba(255,255,255,0.07);
          border-radius: 13px; cursor: pointer; transition: all 0.17s;
        }
        ...
        .cp-pay-dot {
          width: 10px; height: 10px; border-radius: 50%; background: #a3e635;
        ...
        @media (max-width: 500px) { .cp-confirm-info { grid-template-columns: 1fr; } }
        .cp-confirm-info-box {
        ...
        .cp-empty-ico {
          ...
          display: flex; align-items: center; justify-content: center; color: rgba(163,230,53,0.6);
        }
        ...
        .cp-shop-link {
          display: inline-flex; align-items: center; gap: 6px; padding: 0.85rem 1.5rem;
          background: linear-gradient(135deg, #a3e635 0%, #84cc16 100%); border: none; border-radius: 12px;
          color: #052e16; font-family: 'Syne', sans-serif; font-size: 0.95rem; font-weight: 800;
          cursor: pointer; text-decoration: none; transition: all 0.15s;
        }
        ...
        .cp-toast {
          ...
          padding: 0.8rem 1.2rem; border-radius: 10px; font-size: 0.9rem; font-weight: 500;
          font-family: 'DM Sans', sans-serif; border: 1px solid;
          backdrop-filter: blur(14px); animation: cp-toast 0.25s ease;
        ...
        <main className="cp-main">
          ...
          {loading ? (
            <div className="cp-empty">
              <div className="cp-empty-ico"><SpinnerRing size={28} color="#a3e635" /></div>
              <p style={{ color: '#4b5563' }}>Loading your cart…</p>
            </div>
          ) : (
            <>
              {/* ── Progress Step Bar (hidden on empty cart screen) ── */}
              ...
              {checkoutStep === 'cart' && (
                cartItems.length === 0 ? (
                  ...
                  <div className="cp-layout">
                          ...
                          <div className="cp-item-body">
                            ...
                            <div className="cp-item-foot">
                              <div className="cp-qty">
                                <button className="cp-qty-btn" disabled={updating === item.id || (item.quantity || 1) <= 1} onClick={() => updateQuantity(item.id, (item.quantity || 1) - 1)}><MinusIcon /></button>
                                <span className="cp-qty-val">{item.quantity || 1}</span>
                                <button className="cp-qty-btn" disabled={updating === item.id} onClick={() => updateQuantity(item.id, (item.quantity || 1) + 1)}><PlusIcon /></button>
                              </div>
                    ...
                    {/* Detailed Bill Details directly below items */}
                    <div className="cp-glass cp-bill-details">
                      <h3 className="cp-bill-title">Bill Details</h3>
                      
                      <div className="cp-bill-row">
                        <span className="cp-bill-lbl">Item Total ({cartItems.length} items)</span>
                        <span className="cp-bill-val">₹{subtotal.toFixed(2)}</span>
                      </div>
                      
                      <div className="cp-bill-row">
                        <span className="cp-bill-lbl">Delivery Charges</span>
                        <span className="cp-bill-val">{deliveryFee === 0 ? <span style={{ color: '#a3e635' }}>FREE</span> : `₹${deliveryFee}`}</span>
                      </div>
                      
                      <div className="cp-bill-divider" />
                      
                      <div className="cp-bill-total">
                        <span className="cp-bill-total-lbl">Grand Total</span>
                        <span className="cp-bill-total-val">₹{total.toFixed(2)}</span>
                      </div>

                      {deliveryFee > 0 && (
                        <div className="cp-free-banner">
                          <p className="cp-free-txt">Add <span className="cp-free-amt">₹{(500 - subtotal).toFixed(0)}</span> more for free delivery!</p>
                        </div>
                      )}

                      <button className="cp-cta-btn" style={{ marginTop: '1.5rem' }} onClick={() => setCheckoutStep('address')}>
              ...
              {checkoutStep === 'address' && (
                <div className="cp-glass">
                  <AddressStep
                    ...
                    onConfirm={(data) => { setAddressData(data); setCheckoutStep('payment'); }}
                  />
                </div>
              )}
                  ...
                  <div className="cp-pay-options">
                    {[
                      ...
                      <div key={opt.id} className={`cp-pay-opt${paymentMethod === opt.id ? ' selected' : ''}`} onClick={() => setPaymentMethod(opt.id)}>
                        ...
                        <div className="cp-pay-info">
                          <div className="cp-pay-name">{opt.name}</div>
                          <div className="cp-pay-desc">{opt.desc}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    {cartItems.map(item => (
                      <div key={item.id} className="cp-confirm-item">
                        <div>
                          ...
                          <div className="cp-confirm-item-meta">
                            Qty {item.quantity}{getVariantName(item) ? ` · ${getVariantName(item)}` : ''}
                          </div>
                        </div>
                  ...
                  <div className="cp-bill-total" style={{ marginBottom: '1rem' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '0.8rem', color: '#6b7280' }}>
                        {paymentMethod === 'cod' ? 'You\'ll pay on delivery' : 'Secure payment via Razorpay'}
                      </div>
                    </div>
                    <span className="cp-bill-total-val">₹{total.toFixed(2)}</span>
                  </div>

...
function SkeletonOrderCard() {
  return (
    <div className="op-skeleton-card">
      {/* top row */}
      <div className="op-skeleton-top">
        <div className="op-skeleton-left">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            <div className="op-skeleton op-sk-order-id" />
            <div className="op-skeleton op-sk-badge" />
          </div>
          <div className="op-skeleton op-sk-date" />
        </div>
        <div className="op-skeleton-right">
          <div className="op-skeleton op-sk-total-label" />
          <div className="op-skeleton op-sk-total-val" />
          <div className="op-skeleton op-sk-pay-badge" />
        </div>
      </div>

      {/* progress bar placeholder */}
      <div className="op-skeleton-progress-row">
        {[0, 1, 2, 3].map(i => (
          <div key={i} className="op-skeleton-prog-step">
            <div className="op-skeleton op-sk-prog-circle" />
            <div className="op-skeleton op-sk-prog-label" />
          </div>
        ))}
      </div>

      {/* thumbnails */}
      <div className="op-skeleton-thumbs">
        {[0, 1, 2, 3].map(i => (
          <div key={i} className="op-skeleton op-sk-thumb" />
        ))}
      </div>

      {/* footer */}
      <div className="op-skeleton-footer-row">
        <div className="op-skeleton op-sk-items-label" />
        <div style={{ display: 'flex', gap: 8 }}>
          <div className="op-skeleton op-sk-action-btn" />
          <div className="op-skeleton op-sk-action-btn" />
        </div>
      </div>
    </div>
...
export default function OrdersPage() {
  ...
  const [processingReorder, setProcessingReorder] = useState(null); // New state for reorder
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/orders`, { headers: { Authorization: `Bearer ${session.accessToken}` } });
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/cart`, { headers: { Authorization: `Bearer ${session.accessToken}` } })
      .then(r => r.json()).then(d => setCartCount(d.items?.length || 0)).catch(() => {});
  }, [session]);
  ...
  async function downloadInvoice(orderId, invoiceId, e) {
    ...
    try {
      const res = await fetch(`${API_BASE}/orders/${orderId}/invoice`, { headers: { Authorization: `Bearer ${session.accessToken}` } });
    ...
    } catch { showToast('error', 'Could not download invoice'); }
    finally { setInvoiceLoading(null); }
  }

  // ── New Reorder Logic ──
// ── Corrected Reorder Logic ──
  async function handleReorder(order, e) {
    e?.stopPropagation(); 
    if (!session?.accessToken || processingReorder === order.id) return;
    
    setProcessingReorder(order.id);
    ...
    try {
      ...
      router.push('/userdashboard/cart'); // Navigate to checkout automatically
    } catch (err) {
      console.error(err);
      showToast('error', 'Could not reorder some items');
    } finally {
      setProcessingReorder(null);
    }
  }
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .op-skeleton {
          ...
          animation: op-shimmer 1.6s infinite linear;
          border-radius: 8px;
        }
        ...
        .op-skeleton-left { display: flex; flex-direction: column; gap: 0; }
        .op-skeleton-right { display: flex; flex-direction: column; align-items: flex-end; gap: 7px; }

        .op-sk-order-id   { height: 16px; width: 120px; }
        .op-sk-badge      { height: 20px; width: 80px; border-radius: 20px; }
        .op-sk-date       { height: 12px; width: 150px; margin-top: 6px; }
        .op-sk-total-label{ height: 11px; width: 36px; }
        .op-sk-total-val  { height: 22px; width: 80px; }
        .op-sk-pay-badge  { height: 18px; width: 50px; border-radius: 4px; }

        /* skeleton progress */
        .op-skeleton-progress-row {
          padding: 0 1.25rem 0.9rem;
          display: flex; justify-content: space-between; align-items: flex-end;
        }
        .op-skeleton-prog-step {
          display: flex; flex-direction: column; align-items: center; gap: 6px; flex: 1;
        }
        .op-sk-prog-circle { width: 18px; height: 18px; border-radius: 50%; }
        .op-sk-prog-label  { height: 10px; width: 40px; }

        /* skeleton thumbs */
        .op-skeleton-thumbs {
          padding: 0 1.25rem 1rem; display: flex; gap: 0.5rem;
        }
        .op-sk-thumb { width: 52px; height: 52px; border-radius: 10px; flex-shrink: 0; }

        /* skeleton footer */
        .op-skeleton-footer-row {
          padding: 0.75rem 1.25rem;
          border-top: 1px solid rgba(255,255,255,0.05);
          display: flex; align-items: center; justify-content: space-between; gap: 0.75rem;
        }
        .op-sk-items-label { height: 13px; width: 60px; }
        .op-sk-action-btn  { height: 32px; width: 80px; border-radius: 8px; }

        /* ── root ── */
        .op-root {
          min-height: 100vh; background: #080a0f;
          font-family: 'DM Sans', sans-serif; color: #e5e7eb;
        ...
        .op-page-title {
          ...
          font-size: clamp(1.35rem, 3vw, 1.7rem); font-weight: 800;
        ...
        .op-page-count {
          ...
          color: #a3e635; font-family: 'Syne', sans-serif; margin-left: 0.5rem;
        }
        ...
        .op-empty {
          ...
          text-align: center; padding: 5rem 1rem; animation: op-fadein 0.4s ease;
        }
        .op-empty-icon-ring {
          width: 90px; height: 90px; border-radius: 50%;
          background: rgba(163,230,53,0.06); border: 1px solid rgba(163,230,53,0.14);
          display: flex; align-items: center; justify-content: center;
          color: rgba(163,230,53,0.5); margin-bottom: 1.5rem; position: relative;
        }
        .op-empty-icon-ring::before {
          content: ''; position: absolute; inset: -6px; border-radius: 50%;
          border: 1px solid rgba(163,230,53,0.06);
        }
        .op-empty-title { font-family: 'Syne', sans-serif; font-size: 1.25rem; font-weight: 700; color: #fff; margin-bottom: 0.5rem; }
        .op-empty-sub   { font-size: 0.875rem; color: #6b7280; margin-bottom: 1.75rem; max-width: 280px; }
        .op-browse-btn {
          display: inline-flex; align-items: center; gap: 7px; padding: 0.8rem 1.6rem;
          background: linear-gradient(135deg, #a3e635 0%, #84cc16 100%);
          border: none; border-radius: 12px; color: #052e16;
        ...
        .op-list { display: flex; flex-direction: column; gap: 0.875rem; animation: op-fadein 0.35s ease; }
        @keyframes op-fadein { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }

        .op-card {
          background: rgba(12,15,23,0.75); backdrop-filter: blur(16px);
        ...
        .op-card-top { padding: 1.1rem 1.25rem 1rem; display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }
        ...
        .op-card-id { font-family: 'Syne', sans-serif; font-size: 0.9rem; font-weight: 700; color: #fff; }
        .op-card-date { font-size: 0.775rem; color: #4b5563; }
        .op-status-badge {
          display: inline-flex; align-items: center; gap: 5px; padding: 0.28rem 0.75rem;
          border-radius: 20px; font-size: 0.55rem; font-weight: 700;
          text-transform: uppercase; letter-spacing: 0.06em; border: 1px solid;
        }
        .op-status-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
        ...
        .op-card-total { font-family: 'Syne', sans-serif; font-size: 1.2rem; font-weight: 800; color: #a3e635; }
        .op-card-pay-badge {
          font-size: 0.68rem; font-weight: 600; padding: 2px 7px; border-radius: 4px;
          background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); color: #6b7280; letter-spacing: 0.03em;
        }

        .op-progress-bar { padding: 0 1.25rem 0.9rem; }
        .op-progress-track { display: flex; align-items: center; gap: 0; position: relative; }
        .op-progress-step { display: flex; flex-direction: column; align-items: center; gap: 4px; flex: 1; position: relative; z-index: 1; }
        .op-progress-circle {
        ...
        .op-progress-line {
          position: absolute; top: 9px; left: calc(50% + 9px); right: calc(-50% + 9px);
        ...
        .op-progress-label { font-size: 0.6rem; color: #4b5563; font-weight: 500; white-space: nowrap; letter-spacing: 0.02em; }
        .op-progress-step.done   .op-progress-label { color: #34d399; }
        .op-progress-step.active .op-progress-label { color: #a3e635; }

        .op-thumbs-strip { padding: 0 1.25rem 1rem; display: flex; gap: 0.5rem; overflow-x: auto; }
        .op-thumbs-strip::-webkit-scrollbar { display: none; }
        .op-thumb { width: 52px; height: 52px; border-radius: 10px; object-fit: cover; flex-shrink: 0; border: 1px solid rgba(255,255,255,0.07); background: #0c111a; }
        .op-thumb-more {
          width: 52px; height: 52px; border-radius: 10px;
          background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.07);
          display: flex; align-items: center; justify-content: center;
          font-size: 0.72rem; font-weight: 700; color: #6b7280; flex-shrink: 0;
        }
        ...
        .op-btn {
          display: inline-flex; align-items: center; gap: 5px; padding: 0.45rem 0.85rem;
          border-radius: 8px; font-family: 'DM Sans', sans-serif;
          font-size: 0.775rem; font-weight: 500; cursor: pointer; transition: all 0.15s; text-decoration: none; border: 1px solid;
        }
        .op-btn-ghost { background: rgba(255,255,255,0.04); border-color: rgba(255,255,255,0.08); color: #9ca3af; }
        .op-btn-ghost:hover { background: rgba(163,230,53,0.07); border-color: rgba(163,230,53,0.2); color: #a3e635; }
        .op-btn-primary { background: rgba(163,230,53,0.1); border-color: rgba(163,230,53,0.22); color: #a3e635; }
        ...
        .op-overlay {
          ...
          display: flex; align-items: center; justify-content: center;
          padding: 1rem; animation: op-fadein 0.18s ease;
        }
        .op-modal {
          background: #0c0f18; border: 1px solid rgba(255,255,255,0.09);
        ...
        @keyframes op-modal-in { from { opacity: 0; transform: scale(0.94) translateY(16px); } to { opacity: 1; transform: scale(1) translateY(0); } }
        .op-modal-head {
          display: flex; align-items: center; justify-content: space-between;
          padding: 1.25rem 1.5rem; border-bottom: 1px solid rgba(255,255,255,0.07);
          position: sticky; top: 0; z-index: 2;
          background: rgba(12,15,24,0.98); backdrop-filter: blur(20px); border-radius: 22px 22px 0 0;
        }
        .op-modal-head-left { display: flex; flex-direction: column; gap: 3px; }
        .op-modal-head-title { font-family: 'Syne', sans-serif; font-size: 1.05rem; font-weight: 800; color: #fff; }
        ...
        .op-status-hero-icon { width: 42px; height: 42px; border-radius: 11px; display: flex; align-items: center; justify-content: center; font-size: 1.3rem; flex-shrink: 0; }
        .op-status-hero-label { font-family: 'Syne', sans-serif; font-size: 0.95rem; font-weight: 700; }
        .op-status-hero-sub { font-size: 0.775rem; opacity: 0.7; margin-top: 2px; }
        .op-tl-wrap { margin-bottom: 1.5rem; }
        .op-tl-section-label { font-size: 0.68rem; font-weight: 700; letter-spacing: 0.1em; color: #4b5563; text-transform: uppercase; margin-bottom: 0.9rem; }
        .op-tl { display: flex; flex-direction: column; gap: 0; padding-left: 0.25rem; }
        .op-tl-item { display: flex; gap: 1rem; position: relative; }
        ...
        .op-tl-dot {
          ...
          display: flex; align-items: center; justify-content: center; flex-shrink: 0; z-index: 1; transition: all 0.2s;
        }
        ...
        .op-tl-line { width: 2px; flex: 1; min-height: 30px; background: rgba(255,255,255,0.06); margin: 3px 0; }
        ...
        .op-tl-title { font-size: 0.875rem; font-weight: 600; color: #e5e7eb; margin-bottom: 2px; }
        ...
        .op-modal-divider { height: 1px; background: rgba(255,255,255,0.06); margin: 1.5rem 0; }
        .op-modal-items-label { font-size: 0.68rem; font-weight: 700; letter-spacing: 0.1em; color: #4b5563; text-transform: uppercase; margin-bottom: 0.9rem; }
        .op-modal-items-list { display: flex; flex-direction: column; gap: 0; }
        .op-mi { display: flex; gap: 0.9rem; padding: 0.85rem 0; border-bottom: 1px solid rgba(255,255,255,0.05); align-items: center; }
        .op-mi:last-child { border-bottom: none; }
        .op-mi-img { width: 64px; height: 64px; border-radius: 11px; object-fit: cover; flex-shrink: 0; background: #0c111a; border: 1px solid rgba(255,255,255,0.07); }
        .op-mi-body { flex: 1; min-width: 0; }
        .op-mi-name { font-family: 'Syne', sans-serif; font-size: 0.875rem; font-weight: 700; color: #f3f4f6; margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .op-mi-variant { display: inline-block; font-size: 0.7rem; color: #9ca3af; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); padding: 2px 7px; border-radius: 5px; margin-bottom: 5px; }
        ...
        .op-mi-price { margin-left: auto; font-family: 'Syne', sans-serif; font-size: 1rem; font-weight: 800; color: #a3e635; }
        .op-info-row { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; margin-bottom: 1.25rem; }
        .op-info-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 12px; padding: 0.9rem 1rem; }
        .op-info-card-label { display: flex; align-items: center; gap: 6px; font-size: 0.68rem; font-weight: 700; letter-spacing: 0.08em; color: #4b5563; text-transform: uppercase; margin-bottom: 0.6rem; }
        .op-info-card-label svg { opacity: 0.6; }
        .op-info-val { font-size: 0.85rem; color: #d1d5db; line-height: 1.5; font-weight: 400; }
        .op-info-val strong { color: #e5e7eb; font-weight: 600; }
        .op-sum { background: rgba(255,255,255,0.025); border: 1px solid rgba(255,255,255,0.07); border-radius: 14px; padding: 1.1rem 1.25rem; margin-bottom: 1.25rem; }
        .op-sum-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.65rem; font-size: 0.875rem; }
        ...
        .op-sum-total-row { display: flex; justify-content: space-between; align-items: center; }
        ...
        .op-modal-footer { display: flex; gap: 0.6rem; }
        .op-mfbtn {
          flex: 1; display: flex; align-items: center; justify-content: center; gap: 6px; padding: 0.8rem;
          border-radius: 11px; font-family: 'DM Sans', sans-serif; font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: all 0.15s; border: 1px solid;
        }
        .op-mfbtn-ghost { background: rgba(255,255,255,0.04); border-color: rgba(255,255,255,0.08); color: #9ca3af; }
        .op-mfbtn-ghost:hover { background: rgba(255,255,255,0.08); border-color: rgba(255,255,255,0.15); color: #e5e7eb; }
        .op-mfbtn-primary { background: rgba(163,230,53,0.1); border-color: rgba(163,230,53,0.25); color: #a3e635; }
        ...
        .op-toast {
          ...
          backdrop-filter: blur(16px); animation: op-toast-in 0.25s cubic-bezier(0.34,1.56,0.64,1); max-width: 320px;
        }
        ...
        @keyframes op-toast-in { from { opacity: 0; transform: translateY(12px) scale(0.95); } to { opacity: 1; transform: translateY(0) scale(1); } }

        @media (max-width: 560px) { .op-info-row { grid-template-columns: 1fr; } .op-card-actions { flex-wrap: wrap; } .op-btn { flex: 1; justify-content: center; } }
      `}</style>
        ...
        <main className="op-main">
          <header className="op-page-header">
            <div className="op-header-left">
              <Link href="/userdashboard" className="op-back-link"><ArrowLeftIcon /> Dashboard</Link>
          ...
          {loading ? (
            ...
            <div className="op-list">
              {orders.map((order, cardIdx) => {
                ...
                return (
                  <div key={order.id} className="op-card" onClick={() => setSelectedOrder(order)} style={{ animationDelay: `${cardIdx * 0.04}s` }}>
                    ...
                    {items.length > 0 && (
                      <div className="op-thumbs-strip">
                        ...
                        {items.length > 6 && <div className="op-thumb-more">+{items.length - 6}</div>}
                      </div>
                    )}
                    <div className="op-card-footer">
                      <span className="op-card-items-label">{items.length} item{items.length !== 1 ? 's' : ''}</span>
                      <div className="op-card-actions" onClick={e => e.stopPropagation()}>
                        ...
                        <button 
                          ...
                          disabled={processingReorder === order.id}
                          style={{ background: 'rgba(163,230,53,0.15)', borderColor: 'rgba(163,230,53,0.4)' }}
                        >
                          {processingReorder === order.id 
                            ? <div style={{ width: 12, height: 12, border: '1.5px solid rgba(255,255,255,0.2)', borderTopColor: '#a3e635', borderRadius: '50%', animation: 'op-spin .6s linear infinite' }} />
                            : <ReorderIcon />}
                          Reorder
                        </button>
                        ...
                        <button className="op-btn op-btn-ghost" onClick={() => setSelectedOrder(order)}><EyeIcon /> Details</button>
                      </div>
                    </div>
                  </div>
                );
              ...
              <div className="op-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="op-modal-body">
                  <div className="op-status-hero" style={{ background: sm.bg, borderColor: sm.border, color: sm.color }}>
                    <div className="op-status-hero-icon" style={{ background: sm.bg }}><span style={{ fontSize: '1.3rem' }}>{statusEmoji[o.status] || '📦'}</span></div>
                    <div>
                          ...
                          return (
                            <div key={step.key} className={`op-tl-item${isDone ? ' tl-done' : ''}${isActive ? ' tl-active' : ''}`}>
                              ...
                              <div className="op-tl-content">
                                ...
                                <p className="op-tl-sub">{isDone || isActive ? step.sub : 'Pending'}</p>
                              </div>
                            </div>
                          );
                        })}
                  ...
                  <div className="op-info-row">
                    <div className="op-info-card">
                      ...
                      <div className="op-info-val">{o.delivery_address}<br /><strong>Pincode:</strong> {o.pincode}</div>
                    </div>
                    <div className="op-info-card">
                  ...
                  <div className="op-sum">
                    <div className="op-sum-row"><span className="op-sum-label">Subtotal ({items.length} items)</span><span className="op-sum-val">₹{parseFloat(o.total).toLocaleString('en-IN')}</span></div>
                    <div className="op-sum-row"><span className="op-sum-label">Delivery fee</span><span style={{ color: '#a3e635', fontWeight: 600, fontSize: '0.875rem' }}>FREE</span></div>
                    {o.payment_method !== 'cod' && o.payment_reference && (
                      <div className="op-sum-row"><span className="op-sum-label">Payment ref</span><span className="op-sum-val" style={{ fontSize: '0.75rem', fontFamily: 'monospace', color: '#6b7280' }}>{o.payment_reference}</span></div>
                    )}
                  ...
                  <div className="op-modal-footer">
                    {/* ── REORDER BUTTON (MODAL) ── */}
                    <button 
                      className="op-mfbtn op-mfbtn-primary" 
                      onClick={(e) => handleReorder(o, e)}
                      disabled={processingReorder === o.id}
                    >
                      {processingReorder === o.id 
                        ? <div style={{ width: 14, height: 14, border: '1.5px solid rgba(255,255,255,0.2)', borderTopColor: '#a3e635', borderRadius: '50%', animation: 'op-spin .6s linear infinite' }} />
                        : <ReorderIcon />}
                      Order Again
                    </button>

                    <button className="op-mfbtn op-mfbtn-ghost" onClick={() => downloadInvoice(o.id, o.invoice_id)}>
                      {invoiceLoading === o.id ? <div style={{ width: 14, height: 14, border: '1.5px solid rgba(255,255,255,0.2)', borderTopColor: '#9ca3af', borderRadius: '50%', animation: 'op-spin .6s linear infinite' }} /> : <InvoiceIcon />}
                      Invoice
                    </button>
                    <button className="op-mfbtn op-mfbtn-ghost" style={{ flex: 0.5 }} onClick={() => setSelectedOrder(null)}>Close</button>
                  </div>
                </div>
              </div>
            </div>

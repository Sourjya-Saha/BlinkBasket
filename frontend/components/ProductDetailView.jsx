...
// ProductDetailView.js — update the prop signature
export default function ProductDetailView({ product, onAddToCart, addingToCart, cartVersion = 0 }) {
  ...
  // ── How many units of this product/variant are already in the cart ──
  ...
  // ── Derived stock & price based on selected variant ──
  ...
  // How many more can still be added (total available minus what's already in cart)
  const remaining = Math.max(0, totalStock - alreadyInCart);

  // ── Fetch cart to know how many units are already reserved ──
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/cart`, {
      ...
      .then(data => {
        ...
        // Find cart lines that match this product + currently selected variant
        ...
        // Clamp qty if it now exceeds what's actually still available
        setQuantity(q => Math.min(q, Math.max(1, totalStock - inCart)));
      })
      .catch(() => {});
  // Re-run when session, product, or selected variant changes
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [session, product.id, selectedVariant?.id, cartVersion]);

  // ── On mount: check existing stock alert subscription ──
  useEffect(() => {
    if (!session?.accessToken) return;
  ...
  const handleVariantClick = v => {
    // Reset qty to 1 when switching variants — cart check re-runs via useEffect
  ...
  // Add to cart is only valid when there is remaining quota
  ...
  return (
    <div className="pdv-root">
      <style>{`
        ...
        /* Banner shown when all stock is already in cart */
        .pdv-cart-limit {
          display: flex; align-items: center; gap: 8px;
          padding: 0.7rem 1rem; border-radius: 10px;
          background: rgba(251,146,60,0.08); border: 1px solid rgba(251,146,60,0.25);
          font-size: 0.8rem; font-weight: 600; color: #fb923c;
          animation: pdv-msg-in 0.2s ease;
        }
        .pdv-cart-limit-dot { width: 7px; height: 7px; border-radius: 50%; background: #fb923c; flex-shrink: 0; }

        /* ── BLINKIT-STYLE NOTIFY BUTTON ── */
        .pdv-notify-wrap { display: flex; flex-direction: column; gap: 0.6rem; width: 100%; }
        .pdv-notify-btn {
          position: relative; width: 100%; height: 52px; border-radius: 14px; border: none;
          cursor: pointer; font-family: 'Syne', sans-serif; font-size: 0.9rem; font-weight: 800;
          display: flex; align-items: center; justify-content: center; gap: 9px;
          transition: all 0.25s cubic-bezier(0.34, 1.56, 0.64, 1);
          overflow: hidden; letter-spacing: 0.01em;
        }
        .pdv-notify-btn.state-idle {
          background: linear-gradient(135deg, #a3e635 0%, #84cc16 100%); color: #052e16;
          animation: pdv-pulse-ring 2s ease-in-out infinite;
        }
        .pdv-notify-btn.state-idle:hover {
          transform: translateY(-2px) scale(1.02);
          box-shadow: 0 10px 30px rgba(163,230,53,0.4); animation: none;
        }
        .pdv-notify-btn.state-idle .pdv-btn-bell {
          animation: pdv-bell-ring 2.5s ease-in-out infinite; transform-origin: top center;
        }
        .pdv-notify-btn.state-loading {
          background: rgba(163,230,53,0.15); border: 1.5px solid rgba(163,230,53,0.35);
          color: #a3e635; cursor: not-allowed;
        }
        .pdv-notify-btn.state-subscribed {
          background: linear-gradient(135deg, #059669 0%, #047857 100%); color: #d1fae5;
          animation: pdv-subscribed-pulse 2.5s ease-in-out infinite;
        }
        .pdv-notify-btn.state-subscribed:hover {
          background: linear-gradient(135deg, #dc2626 0%, #b91c1c 100%);
          color: #fee2e2; animation: none; transform: translateY(-1px);
        }
        ...
        .pdv-notify-btn.state-unsubscribing {
        ...
        .pdv-notify-btn.state-idle::before {
          content: ''; position: absolute; top: 0; left: -100%; width: 60%; height: 100%;
          background: linear-gradient(90deg, transparent, rgba(255,255,255,0.25), transparent);
          transform: skewX(-20deg); animation: pdv-shimmer 3s ease-in-out infinite;
        }
        @keyframes pdv-shimmer { 0% { left: -100%; } 40%, 100% { left: 160%; } }
        .pdv-btn-spinner {
          width: 16px; height: 16px; border-radius: 50%;
          border: 2.5px solid rgba(163,230,53,0.25); border-top-color: #a3e635;
          animation: pdv-spin 0.65s linear infinite; flex-shrink: 0;
        }
        .pdv-btn-spinner-red {
          width: 16px; height: 16px; border-radius: 50%;
          border: 2.5px solid rgba(248,113,113,0.25); border-top-color: #f87171;
          animation: pdv-spin 0.65s linear infinite; flex-shrink: 0;
        }
        .pdv-notify-msg {
          font-size: 0.78rem; font-weight: 600; text-align: center;
          padding: 0.45rem 0.8rem; border-radius: 8px; animation: pdv-msg-in 0.2s ease;
        }
          ...
          <div className="pdv-actions">
            {totalStock > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                ...
                {/* ── Cart-limit warning (all stock already reserved) ── */}
                ...
                <div className="pdv-actions-row">
                  <div className="pdv-qty">
                    ...
                    {/* + button — hard cap at `remaining` */}
                    <button
                      className="pdv-qty-btn"
                      onClick={() => setQuantity(q => Math.min(remaining, q + 1))}
                      disabled={quantity >= remaining}
                      title={quantity >= remaining ? `Only ${remaining} more unit${remaining !== 1 ? 's' : ''} can be added` : ''}
                    >+</button>
                  </div>
                  ...
                  <button
                    ...
                    title={!canAddToCart ? 'Maximum available stock already in your cart' : ''}
                  >
                    <ShoppingBag />
              ...
              /* ── OUT OF STOCK: Blinkit-style notify toggle ── */
              <div className="pdv-notify-wrap">
                ...
                <button
                  className={`pdv-notify-btn state-${
                    notifyState === 'unsubscribing' ? 'unsubscribing'
                ...
                {notifyMsg && (
                  <div className={`pdv-notify-msg ${notifyState === 'idle' ? 'info' : 'success'}`}>
                    {notifyMsg}
                  </div>
                )}
              </div>
